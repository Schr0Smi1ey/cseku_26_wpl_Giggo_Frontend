  ...
  return (
    <div className="mx-auto max-w-5xl pb-16">
      ...
      <header className="mt-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        {actions.length > 0 && <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-4">{actions.map((status) => { const Icon = icons[status]; return <Button key={status} size="sm" variant={status === 'cancelled' ? 'danger' : status === 'completed' ? 'primary' : 'secondary'} onClick={() => setAction(status)}><Icon className="h-4 w-4" /> {ACTION_LABELS[status]}</Button>; })}</div>}
      </header>
